...
namespace CSC317PassManagerP2Starter.Modules
{
    ...
    public class PasswordCrypto
    {
        ...
        public static byte[] Encrypt(string text, byte[] item1, byte[] item2, Tuple<byte[], byte[]> key)
        {
        ...
        public static string Decrypt(byte[] cipher, byte[] key1, Tuple<byte[], byte[]> key)
        {
            using (Aes aes = Aes.Create())
            {
                aes.Key = key.Item1;
